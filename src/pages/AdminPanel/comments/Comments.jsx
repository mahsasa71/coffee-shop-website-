...
export default function Comments() {
    ...
    try {
      ...
      if (data) {
        Object.keys(data).forEach((productId) => {
          ...
          const productName = product.name || "بدون نام محصول";

          if (product.comments) {
            Object.keys(product.comments).forEach((commentId) => {
              const c = product.comments[commentId];
              allComments.push({
                id: commentId,
                productId,
                productName,
                username: c.username || "بدون نام کاربر",
                comment: c.comment || c.text || c.body || "بدون متن",
                reply: c.reply || "",
      ...
      swal("خطا در دریافت کامنت‌ها", { icon: "error" });
    }
  };
  ...
  const saveReply = async () => {
    ...
    const path = `${FIREBASE_URL}/products/${currentComment.productId}/comments/${currentComment.id}.json`;
  ...
  const toggleAnswered = async (comment) => {
    const path = `${FIREBASE_URL}/products/${comment.productId}/comments/${comment.id}.json`;
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-4">کامنت‌های محصولات</h2>
          ...
          {comments.length > 0 ? (
            comments.map((c, index) => (
              <tr key={c.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-2 text-center">{c.productName}</td>
      ...
      {replyModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg w-full max-w-md shadow-lg">
            <h3 className="text-lg font-bold mb-2">پاسخ به {currentComment.username}</h3>
            <p className="mb-2">
              کامنت: <span className="font-medium">{currentComment.comment}</span>
            </p>
            <textarea
              value={replyText}
